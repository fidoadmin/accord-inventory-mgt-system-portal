...
import ManufacturerAddOverlay from "./ManufacturerAddOverlay";
import { useManufacturerList } from "@/app/hooks/manufacturer/useManufacturerList";
import { useDeleteManufacturer } from "@/app/hooks/manufacturer/useManufacturerDelete";
import {
...
const ManufacturerMaintenance = () => {
  ...
  const { mutate: addOrUpdateManufacturer } = useAddOrUpdateManufacturer();
  const {
    data: ManufacturerData,
    error: ManufacturerError,
    isLoading: ManufacturerLoading,
  } = useManufacturerList(authKey || "", {
    page: currentPage,
    limit: itemsPerPage,
    search: searchTerm,
    sortBy: sortBy,
      ...
      return menu.SideBarDropdowns?.flatMap((child) => {
        if (child.Name === "Organization" && child.PageDropdowns) {
          return child.PageDropdowns.filter(
            (dropdown) => dropdown.Name === "Manufacturer"
          );
        }
        return [];
      });
  ...
  const totalCount = ManufacturerData?.totalCount || 0;
  ...
  const handleEditManufacturer = (companyId: string, company: any) => {
  ...
  const handleSaveCompany = async () => {
    if (editableCompany) {
      const payload: AddOrUpdateManufacturerPayloadInterface = {
        Id: editingCompanyId,
      ...
      try {
        ...
        if (error instanceof Error) {
          toast.error(`Error updating manufacturer: ${error.message}`);
        } else {
          toast.error("Unknown error updating manufacturer");
        }
      }
    }
  };
  ...
  const handelSuccess = async (newManufacturer: any) => {
    if (newManufacturer) {
      setCompanies((prevManufacturer) => [
        newManufacturer,
        ...prevManufacturer,
      ]);
      setSortBy("created");
  ...
  return (
    <>
            ...
            <table className="w-full border-collapse table-auto">
              ...
              <tbody>
                {ManufacturerLoading ? (
                  ...
                  ManufacturerData?.data.map((company) => (
                    <tr
                          ...
                          <>
                            ...
                            {accessDetails &&
                              accessDetails[0]?.CanDelete === "1" && (
                                <button
                                  onClick={() =>
                                    handleDeleteManufacturer(company.Id)
                                  }
                                  className="mr-2 text-error"
                                >
                                  <DeleteRounded />
      ...
      {(ManufacturerData?.data.length ? ManufacturerData?.data.length : 0) >
        0 && (
        <Pagination
          currentPage={currentPage}
          totalPages={Math.ceil(
            (ManufacturerData?.totalCount || 0) / itemsPerPage
          )}
