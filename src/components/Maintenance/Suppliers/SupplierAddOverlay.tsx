...
import { AddOrUpdateSupplierPayloadInterface } from "@/types/ManufacturerInterface";
...
const SupplierAddOverlay = ({
  ...
  const [formData, setFormData] = useState<AddOrUpdateSupplierPayloadInterface>(
    {
      Id: "",
      Name: "",
      Address: "",
  ...
  const { mutateAsync: addOrUpdateSupplier } = useAddOrUpdateSupplier();
  ...
  const handleCheckboxChange = () => {
    setFormData({
      ...formData,
      SameAsManufacturer: !formData.SameAsManufacturer,
    });
  };
      ...
      >
        ...
        <div className="flex items-center space-x-4 border-2 border-primary rounded-xl w-full px-4 py-2 mt-2 bg-white text-black">
          <label className="flex items-center">
            <p className="text-sm">SameAsManufacturer: </p>
            <input
              type="checkbox"
            ...
            <span
              className={`toggle-switch w-10 h-5 flex items-center bg-gray-300 rounded-full p-1 cursor-pointer ${
                formData.SameAsManufacturer ? "bg-green-500" : "bg-gray-300"
              }`}
            >
              <span
                className={`toggle-dot w-4 h-4 bg-white rounded-full shadow-md transform ${
                  formData.SameAsManufacturer
                    ? "translate-x-5"
                    : "translate-x-0"
                }`}
              ></span>
          ...
          <span>{formData.SameAsManufacturer ? "Yes" : "No"}</span>
        </div>
...
export default SupplierAddOverlay;
